...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Run pipeline with XGBoost + MLflow")
    p.add_argument("--input", type=str, required=True,
                   help="path to CSV (e.g., data/raw/Glaucoma_Data.csv)")
    p.add_argument("--target", type=str, default="casetype")
    p.add_argument("--threshold", type=float, default=0.35)
    p.add_argument("--test_size", type=float, default=0.2)
    p.add_argument("--experiment", type=str, default="Glaucoma prediction")
...
python scripts/run_pipeline.py \                                            
    --input data/raw/Glaucoma_Data.csv \
    --target casetype
